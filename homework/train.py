...
def train(model_name_or_path: str, epochs: int = 5, batch_size: int = 64):
    ...
    class PatchTrainer(L.LightningModule):
        ...
        def training_step(self, x, batch_idx):
            x = x.float() / 255.0 - 0.5  # Convert to [-0.5, 0.5] range
        ...
        def validation_step(self, x, batch_idx):
            ...
            if batch_idx == 0:
                self.logger.experiment.add_images(
                    "input", x[:64].clamp(min=0, max=1).permute(0, 3, 1, 2), self.global_step
                )
                self.logger.experiment.add_images(
                    "prediction", x_hat[:64].clamp(min=0, max=1).permute(0, 3, 1, 2), self.global_step
                )
            return loss

        def configure_optimizers(self):
            return torch.optim.AdamW(self.parameters(), lr=1e-4)  # Lower learning rate for fine-tuning
